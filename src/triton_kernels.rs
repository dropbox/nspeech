...
//!
//! Apple Silicon: loads pre-compiled .metallib binaries — zero runtime compilation.
//! Intel: embeds MSL source, compiles at runtime (cross-compiled metallibs not portable).
//! All generated by kernels/build.py + ninja.
...
// Apple Silicon: embed pre-compiled .metallib binaries (zero runtime compilation).
...
mod kernel_data {
    ...
    pub const IS_METALLIB: bool = true;
}

// Intel: embed MSL source text (must compile at runtime on target GPU).
#[cfg(target_arch = "x86_64")]
mod kernel_data {
    macro_rules! embed_msl {
        ($($name:literal),* $(,)?) => {
            pub fn load_kernel(name: &str) -> Option<&'static [u8]> {
                match name {
                    $($name => Some(include_str!(concat!(
                        "../kernels/out/intel/", $name, ".metal"
                    )).as_bytes()),)*
                    _ => None,
                }
            }
    ...
    kernel_list!(embed_msl);
    pub const IS_METALLIB: bool = false;
}
...
impl TritonKernels {
    ...
    pub fn load(metal_device: &MetalDevice) -> Result<Self> {
        ...
        let load = |name: &str, func_name: &str| -> Result<ComputePipeline> {
            ...
            let lib = if kernel_data::IS_METALLIB {
                device.new_library_with_data(data)
                    .map_err(|e| anyhow::anyhow!("Failed to load metallib {name}: {e}"))?
            } else {
                let source = std::str::from_utf8(data)
                    .map_err(|e| anyhow::anyhow!("Invalid UTF-8 in MSL {name}: {e}"))?;
                device.new_library_with_source(source, None)
                    .map_err(|e| anyhow::anyhow!("Failed to compile MSL {name}: {e}"))?
            };
            let func = lib
