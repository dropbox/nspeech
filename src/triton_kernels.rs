...
//!
//! Loads pre-compiled .metallib binaries (generated by kernels/build.py + ninja).
//! Metallibs are embedded at compile time via include_bytes! — zero runtime compilation.
...
// Embed pre-compiled .metallib binaries at compile time.
// Uses cfg(target_arch) to select Apple Silicon vs Intel variants.
macro_rules! embed_metallibs {
    ($dir:literal; $($name:literal),* $(,)?) => {
        fn embedded_metallib(name: &str) -> Option<&'static [u8]> {
            match name {
                $($name => Some(include_bytes!(concat!(
                    "../kernels/out/", $dir, "/", $name, ".metallib"
                ))),)*
                _ => None,
            }
        }
...
impl TritonKernels {
    /// Load kernels from embedded pre-compiled .metallib binaries.
    pub fn load(metal_device: &MetalDevice) -> Result<Self> {
        ...
        let load = |name: &str, func_name: &str| -> Result<ComputePipeline> {
            eprint!("    {name}...");
            let data = embedded_metallib(name)
                .ok_or_else(|| anyhow::anyhow!("No embedded metallib for {name}"))?;
            let lib = device
