//! Column-tiled quantized matmul + fbgemm bf16 packed GEMM.
//!
...
//! matmul (Accelerate BLAS on macOS, Metal on aarch64).
//!
//! Based on rust-warp/src/fused_matmul.rs.
...
/// fbgemm-rs bf16 packed GEMM on CPU, otherwise plain candle matmul
/// (Accelerate BLAS on macOS CPU, Metal on aarch64).
pub enum MatMul {
    QTensor(Arc<QTensor>),
    #[cfg(feature = "fbgemm")]
...
impl MatMul {
    ...
    /// Uses fbgemm-rs bf16 packed GEMM when on CPU (halves weight memory),
    /// otherwise plain candle matmul (e.g. Metal GPU handles its own matmul).
    pub fn from_tensor(t: Tensor) -> Self {
        #[cfg(feature = "fbgemm")]
        {
            if matches!(t.device(), candle_core::Device::Cpu) {
                ...
                return Self::PackedBf16(Arc::new(packed));
            }
        }
        Self::Tensor(t)
    }
