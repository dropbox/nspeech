"""
Download Parakeet Streaming TDT (Transducer) model from Hugging Face and convert to safetensors.

This script downloads the cache-aware streaming variant of Parakeet TDT:
- nvidia/nemotron-speech-streaming-en-0.6b
- Cache-Aware FastConformer-RNNT with zero overlapping computations
- Configurable chunk sizes (70ms to 1190ms)
...
This script:
1. Downloads parakeet-streaming-tdt from Hugging Face
...
5. Optionally quantizes to GGUF Q8_0 format

Usage:
  python scripts/download_parakeet_streaming_tdt.py --output assets
...
def convert_nemo_config_to_json(yaml_config_path: pathlib.Path) -> Dict[str, Any]:
    ...
    ff_expansion = encoder_cfg.get('ff_expansion_factor', 4)
    ...
    # Check for cache-related config (streaming-specific)
    cache_config = {}
    if 'att_context_size' in encoder_cfg:
        cache_config['att_context_size'] = encoder_cfg['att_context_size']
        print(f"  ✓ Found cache config: att_context_size = {cache_config['att_context_size']}")

    if 'cache_aware' in encoder_cfg:
        cache_config['cache_aware'] = encoder_cfg['cache_aware']
        print(f"  ✓ Cache-aware mode: {cache_config['cache_aware']}")

    config = {
        'architectures': ['ParakeetStreamingTransducer'],
        'model_type': 'parakeet_streaming_transducer',
        'vocab_size': decoder_cfg.get('vocab_size', 8192),
        'blank_id': decoder_cfg.get('blank_id', 0),
        'joint_vocab_size': 8198,  # Joint output includes special tokens
        'encoder_config': {
            'hidden_size': d_model,
        ...
        'streaming_config': cache_config,  # Add streaming-specific config
    }
...
def quantize_tdt_model(cache_dir: pathlib.Path, safetensors_path: pathlib.Path) -> pathlib.Path | None:
    ...
    """
    print("\nQuantizing Streaming TDT model to GGUF with zstd compression...")

    gguf_path = cache_dir / "parakeet-streaming-tdt-model_q8_0.gguf.zst"
    ...
    try:
        ...
        print(f"  ✓ Quantized Streaming TDT model saved to {gguf_path}")
        return gguf_path
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download and convert Parakeet Streaming TDT model"
    )
    parser.add_argument(
        "--cache",
        default=None,
        help="Cache directory (default: .cache/parakeet-streaming-tdt)",
    )
    parser.add_argument(
        "--assets",
        default="assets",
    ...
    if args.cache:
        ...
        project_root = pathlib.Path(__file__).parent.parent
        cache_dir = project_root / ".cache" / "parakeet-streaming-tdt"

    assets_dir = pathlib.Path(args.assets)
    cache_dir.mkdir(parents=True, exist_ok=True)
    assets_dir.mkdir(parents=True, exist_ok=True)

    print("Parakeet Streaming TDT Model Download & Conversion")
    print("=" * 50)
    ...
    try:
        ...
        print("\nNote: This model may have a different filename or structure.")
        print("Please check: https://huggingface.co/nvidia/nemotron-speech-streaming-en-0.6b")
        return 1
    ...
    # Print streaming config if found
    if config.get('streaming_config'):
        print("\nStreaming Configuration:")
        for key, value in config['streaming_config'].items():
            print(f"  {key}: {value}")
    ...
    # Compress config
    config_zst = compress_file(config_path, "parakeet-streaming-tdt-config.json.zst")
    compressed_files.append(config_zst)

    # Compress weights
    weights_zst = compress_file(safetensors_path, "parakeet-streaming-tdt-model.safetensors.zst")
    compressed_files.append(weights_zst)

    # Compress tokenizers if they exist
    if files['tokenizer']:
        # Compress tokenizer.model
        tokenizer_model_zst = compress_file(tokenizer_model_path, "parakeet-streaming-tdt-tokenizer.model.zst")
        compressed_files.append(tokenizer_model_zst)

        # Compress tokenizer.json if it was created
        if (cache_dir / "tokenizer.json").exists():
            tokenizer_json_zst = compress_file(cache_dir / "tokenizer.json", "parakeet-streaming-tdt-tokenizer.json.zst")
    ...
    print(f"Assets: {assets_dir}")
    print("\nAssets contents:")
    for p in sorted(assets_dir.glob("parakeet-streaming-tdt-*.zst")):
        size_mb = p.stat().st_size / (1024 * 1024)
        print(f"  - {p.name} ({size_mb:.1f} MB)")

    print("\nNext steps:")
    print("1. Implement cache support in FastConformer")
    print("2. Create streaming model loader")
    print("3. Test with: cargo run --example transcribe_streaming_tdt --release -- audio.wav")
